...
export default {
  ...
  protectInterior: '保护角色内部（连通抠除）',
  protectInteriorDesc:
    '只抠与画面边缘连通的背景，角色内部同色（白衣、浅色装备等）不会被抠成透明。强烈建议保持开启。',
  enclosedMin: '封闭区域扣除阈值',
  enclosedMinDesc:
    '内部未与边缘连通的「同色空洞」（如腋下缝隙、镂空）面积达到此像素数才会被抠掉。调大更保护内部；调到 0 则凡是同色候选都会抠（等同弱化保护）。',
  softness: '羽化半径',
  softnessDesc:
  ...
  defringe: '彻底去白边（去污色/碎边）',
  defringeDesc:
    '开启后会从半透明像素扣掉混入的背景色，并清除贴边的近样本色碎边。下方「边缘收缩」「弱透明剔除」始终独立生效，不依赖本开关。',
  erodePx: '边缘收缩',
  erodePxDesc:
    '把不透明外轮廓向内收缩指定像素（0~15），用于掐掉白边。大尺寸原图建议 2~8；预览缩略时不明显，请放大预览或下载后查看。调太大可能咬掉主体轮廓。',
  alphaCutoff: '弱透明剔除',
  alphaCutoffDesc:
    '透明度低于此阈值的像素直接变全透明，去掉羽化淡残影。始终独立生效。数值越大清得越狠，过大可能让边缘发硬。',
  sampleRadius: '取色取样半径',
  sampleRadiusDesc:
